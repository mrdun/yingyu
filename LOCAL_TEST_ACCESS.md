...
| `/my-courses`               | 我的课程 (最近学习 + 复习本 / 掌握列表 / 生词本; 游客为登录提示)                  |
| `/membership`               | 会员方案 / 支付入口                                                               |
| `/partner`                  | 推广中心                                                                          |

> 应用外壳 (本轮): 登录后的业务页改用 **工作台外壳 (app-shell)** —— 固定左侧栏 (212px, AppRail)
> 顶替顶部通栏, 右侧是独立滚动的圆角内容面板; `小于 1024px` 时侧栏收成 **56px 工具条 + 抽屉**。
> 未登录的任何页面、落地页 `/`、沉浸式练习 `/game/*` 与协议页 **保持原来的顶部通栏 + 居中单栏**。
>
> 导航调整: 登录态侧栏 (AppRail) 里「开始学习」改名 **「课程广场」**、「学习路线」改名
> **「课程向导」**, 并新增「主页」(`/`) 与「我的课程」(`/my-courses`)。
> 其中「我的课程」整页取代了原来的 **「学习档案」(`/archive`, 已删除)** ——
> 复习本 / 掌握列表 / 生词本三张卡片与「最近学习」都收在这一页;
> 「掌握列表」(`/mastered-elements`) 页面本身保留, 只是不再占侧边栏。
> 游客落地页顶部导航只保留 **功能 / 问题 / 联系我们** 三个页内锚点 (会员功能入口只在登录态出现)。
>
> 用户端外链清理 (与上面同一轮): 落地页导航的「文档」、用户菜单的「帮助文档」与「建议反馈」、
> 「快速上手」里的 B 站视频、以及创始会员邀请函横幅 (含「查看详情」) 全部下线 —— 用户端不再有任何
> 指向第三方/上游的可点击外链。发音用的有道接口、分享图占位图、备案/GitHub 注释块按原样保留。
