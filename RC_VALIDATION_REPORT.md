...
| 环境变量注入        | ✅ 产物中烘焙了 RC 的 `API_BASE=http://localhost:3001`（本地 Logto 只登记 localhost 回调，故用 localhost 而非 127.0.0.1）                           |
| 残留错误配置        | ✅ 产物中**无**其它环境的值（如生产域名）残留；构建期已加门禁：`nuxt generate/build` 缺 `API_BASE`/`LOGTO_*` 等变量直接失败, 不再静默产出空配置产物 |
